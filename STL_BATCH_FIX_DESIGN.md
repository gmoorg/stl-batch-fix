...
**`shells()` uses `scipy.sparse.csgraph.connected_components`**, which is why
scipy is a hard requirement in `install.sh`. Measured on
...
`tests/tests/test_blender.py` — 32 tests, ~20 s. Most never launch Blender: the module's
...
`tests/tests/test_converter.py` — 20 tests for `libs/converter.py`, ~0.05 s. `convert` is
...
`tests/tests/test_meshfix.py` — 23 tests for `libs/meshfix.py`, ~0.02 s. The library's
...
`tests/tests/test_splitter.py` — 27 tests for `libs/splitter.py`, ~0.02 s. Index arrays
...
`tests/tests/test_scanner.py` — 46 tests for `libs/scanner.py`, ~7 s. Fixtures are built by
...
`tests/tests/test_decimator.py` — 19 tests for `libs/decimator.py`, ~0.4 s. Both rungs are
...
`tests/tests/test_mesh_io.py` — 44 tests for `libs/mesh_io.py`, ~0.7 s. Every file is
...
`tests/tests/test_indicators.py` — 21 tests for `libs/indicators.py`, ~0.01 s. Every file
is an empty touch, since the module tests for existence and never opens
anything.

`tests/tests/test_pool.py` — 24 tests for `libs/pool.py`, ~0.9 s. No meshes, no
...
the system `tests/tests/test_pipeline.py` cannot reach — every item handled exactly once
...
`tests/tests/test_pipeline.py` — 36 end-to-end tests, ~3.8 s:

```bash
../.venv/bin/python tests/tests/test_pipeline.py            # all of it
...
`tests/tests/make_fixtures.py`), not models from the collection — the collection is meant to
