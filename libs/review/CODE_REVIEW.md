...
**Updated 2026-09-21: Blender now runs in the default per-part route.** The table below describes an earlier default; `repairer._repair_part` now composes `meshlab.step_orient`, then `blender.step_blender_repair`, then `meshfix.step_meshfix_repair` (`PART_MESH_STEPS`) — every part reaches this sequence, no defect check gates whether Blender runs on it, but `pipeconfig.ENABLE_BLENDER_PART` (default on) can still disable the Blender step itself for the whole run. Owner decision, not the defect-based routing this section originally called for. Measurements favor Blender for holes and fins, PyMeshFix for non-manifold geometry and winding seams; the missing decision — a selector that sends a part to only the tool its measured defects call for, or an evidence-based order when a part needs both — remains open, tracked in [open issues](../../docs/refactor/open-issues.md). `repairer.blender_part`, the wholesale-replacement wrapper this paragraph originally referred to, has since been removed as redundant: `blender.step_blender_repair` already has the signature `repair(tool=...)` requires.
