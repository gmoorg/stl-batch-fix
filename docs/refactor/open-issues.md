...
- **Move step ordering into `pipeconfig`, with per-step IDs; remove `tool=`.** Owner proposal, 2026-09-21, from a comment above `repairer.Step`: give each step its own ID in a separate enumerator, have `repair()` receive a `tuple[Step, ID]` sequence in its constructor and populate `WHOLE_MESH_STEPS`/`PART_MESH_STEPS`/a `SPLIT_STEP` from it, with the default order living in `pipeconfig` rather than hardcoded in `repairer.py`. This would let steps move around for testing (e.g. reordering, injecting/removing a step) without editing `repairer.py`'s source, and — the owner's stated motivation — remove the need for `repair(tool=...)` as the only way to substitute the per-part sequence, since a caller could instead substitute one entry in the ID-keyed sequence.
  **Landed this session, as a first step toward this, not the finished redesign:** `Step` collapsed from 8 fine-grained members (WELD, CLEAN_NULL_FACES, CLEAN_MERGE_CLOSE, CLEAN_DUPLICATE_FACES, CLEAN_UNREFERENCED, SPLIT, PART, MERGE) to 4 (PREP, SPLIT, PART, MERGE); `WHOLE_MESH_STEPS` changed from `tuple[tuple[Step, Callable], ...]` to `tuple[tuple[str, Callable], ...]`, and `PART_MESH_STEPS` (previously bare `tuple[Callable, ...]`, never `Step`-keyed) took the same `(str, Callable)` shape — which whole-mesh step ran is now only in each `StepResult.detail` string, not `.step`. Not yet done: moving `WHOLE_MESH_STEPS`/`PART_MESH_STEPS` (or their replacement) into `pipeconfig`; the separate per-step ID enumerator; a `SPLIT_STEP` entry so SPLIT joins the same configurable-sequence pattern (SPLIT/MERGE currently stay outside `_run_step`'s uniform contract entirely, by earlier design — they change cardinality, so "one step, one mesh in, one mesh out" does not fit them; whether a `SPLIT_STEP` changes that is itself open); removing `tool=`.
